...
Regras de conduta do usuário (respostas, autorização, entrega): @.claude/REGRAS.md
...
**Rollback aqui não pode escrever no working tree.** O `git push` já publica em PROD pela Vercel e este working tree costuma carregar várias versões pendentes não commitadas. `git checkout <commit> -- App.jsx` destrói esse trabalho; `git revert` conflita com arquivo alterado. Usar `git commit-tree` (`TREE=$(git rev-parse <commit-bom>^{tree})`, `NEW=$(git commit-tree "$TREE" -p HEAD -F msg.txt)`, `git update-ref refs/heads/main "$NEW"`, `git push`) — cria commit novo com o conteúdo do commit bom, sem force e sem tocar no working tree. Conferir com `git diff --stat origin/main <commit-bom>` (vazio) e `git hash-object App.jsx`.
...
## Alertas de vencimento (concluído)

Sistema de alertas da agenda funcionando desde a v5.2.3; comportamento confirmado pelo usuário na v6.2.1. Envia para itens vencidos no mês corrente ainda não quitados e para vencimentos nos próximos N dias (configurável). **Não envia** para status `pago` ou `baixado`, e a reconciliação da agenda não reverte item `baixado` para `pendente`. O rodapé do e-mail mostra a versão, para conferir qual build está no ar. Qualquer ajuste vive em `supabase/functions/send-alerts/index.ts` — ver o deploy nos dois refs no topo deste arquivo.

## Detecção de fatura na importação (v5.5.0)

Ao importar na tela "Importar Extrato", `isFaturaFile()` detecta fatura de cartão em duas camadas: nome do arquivo contendo "fatura" (case-insensitive), ou mais de 40% das descrições no padrão de parcela `XX/YY`. Dispara modal laranja com "Cancelar" ou "Importar mesmo assim"; confirmando, o column mapper abre com banner de aviso persistente. O `handleDetailFile` (fatura dentro do Detalhamento) fica intocado — lá a fatura é esperada. O modal está logo antes do bloco `{/* Confirm clear */}` no `App.jsx`.

## Teste de UI em browser real (CDP)

Sem Playwright no projeto, a UI se testa por Chrome DevTools Protocol num perfil já autenticado em DEV (`marcelo.kling@gmail.com`), fora do repo: perfil em `C:\Users\marce\.claude\test-profiles\cashflow-dev-chrome`, driver em `C:\Users\marce\.claude\test-profiles\cdp.mjs` (WebSocket nativo do Node 24+, sem dependências).

1. `npm run dev` no projeto.
2. `chrome.exe --remote-debugging-port=9333 --user-data-dir="C:\Users\marce\.claude\test-profiles\cashflow-dev-chrome" --no-first-run --no-default-browser-check "http://localhost:5173/"` (ajustar a porta do Vite).
3. `curl http://localhost:9333/json` → `webSocketDebuggerUrl` do target `type: "page"`.
4. `export CDP_WS="ws://..."` e `node cdp.mjs '<js async com evaluate(expr) e send(method,params)>'`.

Cliques: `element.dispatchEvent(new MouseEvent("click",{bubbles:true}))` (funciona com a delegação do React); `checkbox.click()` para inputs. Encadear ação → `setTimeout` 200-300ms → verificação, porque o React não re-renderiza dentro da mesma chamada. Screenshot: `send("Page.captureScreenshot",{format:"png"})`. Ao terminar, `send("Browser.close",{})` em vez de matar o processo, senão o perfil corrompe. Se a sessão expirar, abrir sem headless e pedir login manual ao usuário.
