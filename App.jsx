...
// v8.2.0 — o FITID continua sendo gravado como dado do banco, mas não decide mais
// duplicidade: no Itaú ele é a posição da linha no arquivo e muda a cada export.
export const fitKey = (conta, fitid) => `${conta||""}|${fitid}`;

// v8.2.0 — Conciliação por grupo, no lugar de tentar identificar cada linha.
// Duas linhas realmente iguais (mesma conta, dia, valor e texto) são indistinguíveis:
// o dado que as separaria não existe no arquivo. Mas identidade individual não é
// necessária para evitar duplicidade — basta conferir quantidade: de cada grupo
// conta+data+valor entra só o que o arquivo tiver a mais do que já está gravado.
// Assim, texto reescrito pelo banco, ordem invertida e reemissão do dia não duplicam.

// A conta vem escrita de formas diferentes conforme o formato — o OFX do Itaú traz
// 1618995128 (agência colada) e a planilha do mesmo banco traz 00995128. Os últimos
// seis dígitos significativos são o que os dois têm em comum.
export const contaKey = (conta) => {
  const d = String(conta||"").replace(/\D/g,"").replace(/^0+/,"");
  return d.length > 6 ? d.slice(-6) : d;
};

export const grupoKey = (r) =>
  `${contaKey(r.conta)}|${r.date}|${parseFloat(r.value).toFixed(2)}`;

// Assinatura só de letras: serve para emparelhar a linha certa com a linha certa
// dentro do grupo. Nunca decide se algo é novo — se não casar, cai na contagem.
const assinatura = (s) => String(s).toUpperCase().normalize("NFD").replace(/[̀-ͯ]/g,"")
  .replace(/[^A-Z ]/g," ").replace(/\s+/g," ").trim().split(" ").slice(0,4).join(" ");

export const conciliar = (gravados, linhas) => {
  const grupos = new Map();
  for (const g of gravados) {
    const k = grupoKey(g);
    if (!grupos.has(k)) grupos.set(k, []);
    grupos.get(k).push({ ass: assinatura(g.description), usado: false });
  }
  const novos = [];
  for (const l of linhas) {
    const g = grupos.get(grupoKey(l));
    if (!g) { novos.push(l); continue; }
    const a = assinatura(l.description);
    const alvo = g.find(x => !x.usado && x.ass === a) || g.find(x => !x.usado);
    if (alvo) alvo.usado = true; else novos.push(l);
  }
  return novos;
};

// Linha de saldo não é movimento — vale para OFX, planilha e CSV.
export const ehLinhaDeSaldo = (desc) => /^SALDO\b/i.test(String(desc||"").trim());
...
export const parseOFX = (text) => {
  ...
  for (const [,b] of String(text).matchAll(/<STMTTRN>([\s\S]*?)<\/STMTTRN>/g)) {
    ...
    if (!fitid || d.length<8 || !isFinite(value) || ehLinhaDeSaldo(memo)) continue;
...
// e continuava classificando toda importacao. Agora o Passe 3 pula o que foi removido.
export const localClassify = (desc, customCats = [], hiddenBase = []) => {
  const d = String(desc).toUpperCase().trim();
  const ocultas = new Set((hiddenBase||[]).map(n=>String(n).toUpperCase().trim()));
  ...
  for (const cls of SORTED_CLASSIFICATIONS) {
    const base = cls.d.toUpperCase().trim();
    if (ocultas.has(base)) continue;
    if (d.includes(base)) return { r: cls.r, c: cls.c, sub: null, catId: null, matchedKw: cls.d };
  }
  return null;
};
...
// ══════════════════════════════════════════════════════════════════════════════
// v7.21.0 — tela unica: classificar e decidir a regra permanente no mesmo lugar.
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:16}}>
      <div style={{background:"#162130",borderRadius:16,padding:24,width:"100%",maxWidth:760,border:"1px solid #F5A623",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{fontSize:13,color:"#6B8299",marginBottom:4}}>{rows.length} lançamento(s) não reconhecidos — ajuste, decida o que vira regra e confirme.</div>
        ...
        {rows.map((t,i)=>(
          <div key={i} style={{background:"#0F1923",borderRadius:10,padding:14,marginBottom:10,border:"1px solid #1E2D3D"}}>
            <div style={{fontSize:13,fontWeight:600,marginBottom:3}}>{t.description}</div>
            <div style={{fontSize:11,color:"#6B8299",marginBottom:3}}>
              {t.date} · <span style={{color:Number(t.value)>=0?"#00C9A7":"#E8445A",fontWeight:600}}>{fmt(Number(t.value))}</span>
              {t.razao_social?<> · {t.razao_social}</>:null}
            </div>
            <div style={{fontSize:10.5,color:"#6B8299",marginBottom:10,opacity:0.8}}>
              {t.conta?<>conta {t.conta} · </>:null}origem {t.origin||"extrato"}{t.source_file?<> · {t.source_file}</>:null}
            </div>
            ...
            <div style={{marginTop:12,paddingTop:11,borderTop:"1px solid #1E2D3D"}}>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",marginBottom:regras[i]?.criar?8:0}}>
                <input type="checkbox" checked={!!regras[i]?.criar} style={{cursor:"pointer",flexShrink:0}}
                  onChange={e=>setRegra(i,"criar",e.target.checked)}/>
                <span style={{fontSize:11.5,color:regras[i]?.criar?"#00C9A7":"#6B8299",fontWeight:600}}>Criar regra permanente nas Classificações</span>
              </label>
              {regras[i]?.criar&&(
                <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",paddingLeft:24}}>
                  <input type="text" value={regras[i].nome}
...
export default function App() {
  ...
  // v7.22.1 — o contador mostrava BASE_CLASSIFICATIONS.length fixo: nao descontava as base
  // ocultas nem somava as categorias do usuario, que sao consultadas primeiro na importacao.
  const totalRegrasLocais = useMemo(() => {
    const ocultas = new Set((hiddenBaseCls||[]).map(n=>String(n).toUpperCase().trim()));
    const base = BASE_CLASSIFICATIONS.filter(c=>!ocultas.has(c.d.toUpperCase().trim())).length;
    return base + (customCats||[]).filter(c=>c.rd&&c.classificacao).length;
  }, [hiddenBaseCls, customCats]);
  ...
  // ── Auth ──────────────────────────────────────────────────────────────────
  useEffect(()=>{
    document.title = import.meta.env.DEV ? "CashFlow-DEV" : "CashFlow";
  },[]);
  ...
  const loadTransactions = async () => {
    ...
    // v8.2.0 — a duplicidade passou a ser conciliada por grupo direto sobre `transactions`,
    // então não há mais listas de hash nem de FITID para manter.
  ...
  const filtered = useMemo(()=>{
    ...
    if(searchText.trim()){
      ...
      const baseMatch = t=>
        ...
        (qNorm.includes("cartao")&&t.origin==="fatura");
  ...
  const processColumnMapper = async () => {
    ...
    const parsed = rawRows.map(cols=>{
      ...
      // v8.2.0 — "SALDO ANTERIOR", "SALDO DO DIA" e afins não são movimento. A regra é a
      // mesma do leitor de OFX: se um formato descartar e o outro não, a conciliação
      // por grupo passa a comparar quantidades diferentes para o mesmo dia.
      if(mode==="extrato" && ehLinhaDeSaldo(rawDesc)) return null;
    ...
    if(mode==="extrato"){
      ...
      const newRows = conciliar(transactions, parsed); // v8.2.0
  ...
  const openOFXImport = async (file) => {
    try {
      ...
      const newRows = conciliar(transactions, rows); // v8.2.0
  ...
  const confirmReview = async (reviewed, regras=[]) => {
    ...
    const resumo = `${rows.length} lançamentos revisados e salvos!` +
      (regrasCriadas ? ` ${regrasCriadas} regra(s) criada(s) nas Classificações.` : "");
    if (hits.length) {
      setSimilarSelected(hits.map(t=>t.id));
      setSimilarPending({items:hits, count:rows.length});
      if (regrasCriadas) showToast(`${regrasCriadas} regra(s) criada(s) nas Classificações.`);
    } else {
      showToast(resumo);
      setTab("lancamentos");
    }
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              <div>
                ...
                <div style={{fontSize:13,color:"#6B8299",marginTop:4}}>
                  ...
                  <span style={{fontWeight:600,color:"#E8EDF2"}}> · Total: {fmt(filtered.reduce((s,t)=>s+Number(t.value),0))}</span>
          ...
          <>
            ...
            {aiLoading&&(
              <div style={{...s.card,textAlign:"center",padding:48,marginBottom:20}}>
                ...
                <div style={{fontSize:13,color:"#6B8299",marginTop:8}}>Consultando {totalRegrasLocais} regras locais + Gemini para desconhecidos.</div>
              </div>
            )}
              ...
              <div style={s.card}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                  <div>
                    ...
                    {/* v8.2.0 — explica a regra que vale para qualquer formato. */}
                    <div style={{fontSize:12,marginTop:8,padding:"7px 11px",borderRadius:7,
                      background:"rgba(0,201,167,0.10)",color:"#00C9A7",border:"1px solid rgba(0,201,167,0.3)"}}>
                      ✓ Duplicidade conferida por conta, data e valor: de cada grupo entra só o que este arquivo tem a mais do que já está gravado. Mudança de texto pelo banco, ordem do arquivo e formato (OFX, CSV ou XLSX) não afetam.
                    </div>
                  </div>
                  <div style={{display:"flex",gap:10}}>
                ...
                <table style={s.table}>
                  ...
                  <tbody>
                    {pendingImport.rows.slice(0,20).map((t,i)=>{
                      // v8.2.0 — o status vem da própria conciliação: quem não está em
                      // newRows é repetido. Recalcular aqui daria resultado diferente para
                      // linhas gêmeas dentro do mesmo arquivo.
            ...
            <div style={s.card}>
              ...
              {(()=>{
                ...
                const rows = Object.values(extratos).sort((a,b)=>{
                  ...
                  const isDate = sortHistCol==="min"||sortHistCol==="max"||sortHistCol==="importedAt";
                  const av=isDate?dateToSortable(a[sortHistCol]||""):a[sortHistCol]||"";
                  const bv=isDate?dateToSortable(b[sortHistCol]||""):b[sortHistCol]||"";
